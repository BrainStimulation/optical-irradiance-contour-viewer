...
g <- readMat(
url(
    "https://brainstimulation.github.io/optical-irradiance-contour-webapp/data/grey.mat"
  )
)
w <- readMat(
  url(
    "https://brainstimulation.github.io/optical-irradiance-contour-webapp/data/white.mat"
  )
)
...
# summary data
  card(card_body(verbatimTextOutput("tdata")))
)
# Server logic
server <- function(input, output) {
  ...
  fnameData <- reactive({
    req(
      input$source,
      input$tissue, !is.null(input$drawgridlines), !is.null(input$drawirrsliceline),
      input$power,
      input$threshold,
      input$irrslider,
      input$wavelength
  ...
  draw_contour_r <- reactive({
    req(
      ...
      input$wavelength,
      input$power,
      cData, !is.null(input$drawgridlines),
      sx,
      !is.null(input$drawirrsliceline),
      !is.null(input$irrslider)
    )
    # contour plot
    contour(
    ...
    title(
      ...
      ylab = "Depth (mm)",
      sub = sprintf(
        "Max Irradiance = %.2f mW/mm^2    Vol. Illuminated = %.3f mm^3",
        cData()[1],
        cData()[2]
      )
    )
    ...
    # grid lines
    if (input$drawgridlines == TRUE) {
      abline(v = (seq(-1, 1, length.out = 21)),
             col = 'lightgray',
             lty = 'dotted')
  ...
  draw_contour_f <- function() {
    req(
          ...
          input$power,
          cData, !is.null(input$drawgridlines),
          sx, !is.null(input$drawirrsliceline), !is.null(input$irrslider)
        )
    # contour plot
    contour(
      seq(-1, 1, length.out = 200),
  ...
  output$downloadcontourplot <- downloadHandler(
    filename <- function() {
      req(fnameData)
      sprintf(
        "%s_%s_%snm_P%s_T%s_G%s_S%s_X%smm_CONTOUR.png",
        fnameData()[1],
        fnameData()[2],
        fnameData()[3],
        fnameData()[4],
  ...
  output$downloadirrplot <- downloadHandler(
    filename <- function() {
      req(!is.null(input$irrslicelogplot), fnameData)
      if (input$irrslicelogplot == TRUE) {
        sprintf(
          "%s_%s_%snm_P%s_T%s_G%s_S%s_X%smm_IRRADIANCE_LOG.png",
          fnameData()[1],
          fnameData()[2],
          fnameData()[3],
          fnameData()[4],
        ...
        sprintf(
          "%s_%s_%snm_P%s_T%s_G%s_S%s_X%smm_IRRADIANCE_LIN.png",
          fnameData()[1],
          fnameData()[2],
          fnameData()[3],
          fnameData()[4],
